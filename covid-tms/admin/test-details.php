...
if (strlen($_SESSION['aid'] == 0)) {
  ...
  if (isset($_POST['submit'])) {
    ...
    echo '<script>alert("chỉ định cho kỹ thuật viên thành công.")</script>';
  ...
  if (isset($_POST['takeaction'])) {
    ...
    if ($status == 'Delivered') :
      ...
      if (!in_array($extension, $allowed_extensions)) {
        echo "<script>alert('Định dạng không hợp lệ. Chỉ cho phép định dạng doc / pdf');</script>";
      } else {
...
?>
  ...
  <head>
    ...
    <title>Kovid 19 | Chi tiết xét nghiệm</title>
      ...
      <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
          ...
          <div class="container-fluid">
            ...
            <!-- Page Heading -->
            <h1 class="h3 mb-4 text-gray-800">Chi tiết xét nghiệm# <?php echo intval($_GET['oid']); ?></h1>
            <div class="row">
              <?php
              $testid = intval($_GET['tid']);
                    ...
                    <div class="card-body">
                      ...
                      <table class="table table-bordered" width="100%" cellspacing="0">
                        ...
                        <?php if ($row['FinalReport'] != '') : ?>
                          <tr>
                            <th>Báo cáo</th>
                            <td><a href="reportfiles/<?php echo $row['FinalReport']; ?>" target="_blank">Tải xuống</a></td>
                          </tr>
                      ...
                      ?>
                        <div class="form-group">
                          <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#assignto">Chỉ định cho</button>
                        </div>
                        <?php else :
                        ...
                        if ($rstatus == 'Assigned' || $rstatus == 'On the Way for Collection' || $rstatus == 'Sample Collected' || $rstatus == 'Sent to Lab') : ?>
                          <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#takeaction">Thực hiện phân loại</button>
                      <?php
                        endif;
                      endif; ?>
                    </div>
...
where tblreporttracking.OrderNumber='$orderid'");
          ...
          <div class="row">
            <div class="col-lg-12">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary" align="center">Lịch sử theo dõi xét nghiệm</h6>
                </div>
                <div class="card-body">
                  <?php if ($num > 0) {
                  ?>
                  ...
                  <?php
                    ...
                    <h4 align="center" style="color:red"> Không có lịch sử theo dõi </h4>
                  <?php } ?>
    ...
    <div id="takeaction" class="modal fade" role="dialog">
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h4 align="left">Thực hiện phân loại</h4>
            <button type="button" class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
            <form method="post" enctype="multipart/form-data">
              <p> <select class="form-control" name="status" id="status" required="true">
                  ...
                  <?php if ($reportstatus == 'Assigned') : ?>
                    <option value="On the Way for Collection">Đang trên đường thu thập</option>
                    <option value="Sample Collected">Mẫu đã thu thập</option>
                    <option value="Sent to Lab">Đã gửi đến Phòng thí nghiệm</option>
                    <option value="Delivered">Đã xét nghiệm có kết quả</option>
              ...
              <p id='reportfile'> Chú ý <span style="color:red; font-size:12px;">(Chỉ cho phép định dạng tài liệu và PDF)</span> <input type="file" name="report" id="report"></p>
              <p>
                <textarea name="remark" class="form-control" required="true" placeholder="Ghi chú (Tối đa 500 ký tự)" maxlength="500" rows="5"></textarea>
              </p>
              <p>
